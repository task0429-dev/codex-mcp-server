...
// ── Visionary Tab ─────────────────────────────────────────────────────────────
// All Visionary code lives in visionary-*.ts/tsx files in this directory.
// This file only re-exports VisionaryPage for App.tsx routing.

import { VisionaryUniverse } from "./visionary-canvas";

export function VisionaryPage({ data: _data, actions: _actions, openRoute }: PageProps) {
  return <VisionaryUniverse openRoute={openRoute} />;
}


// ── Agent Dock ────────────────────────────────────────────────────────────────
const DOCK_AGENT_COLORS: Record<string, string> = {
  abdi: "#ef4444", dame: "#f59e0b", ayub: "#3b82f6", ahmed: "#84cc16",
  atlas: "#06b6d4", rex: "#22c55e", prime: "#8b5cf6", sygma: "#ec4899",
  codex: "#e2e8f0", claude: "#f97316",
};
const DOCK_AGENTS = [
  { id: "abdi", name: "Abdi", role: "CEO" },
  { id: "dame", name: "Dame", role: "Operator" },
  { id: "ayub", name: "Ayub", role: "Builder" },
  { id: "ahmed", name: "Ahmed", role: "Memory" },
  { id: "atlas", name: "Atlas", role: "Marketing" },
  { id: "rex", name: "Rex", role: "Infra" },
  { id: "prime", name: "Prime", role: "Trading" },
  { id: "sygma", name: "Sygma", role: "Ops" },
  { id: "codex", name: "Codex", role: "Execution" },
  { id: "claude", name: "Claude", role: "Assistant" },
];

function AgentDock({ agents, onOpenAgent }: { agents: any[]; onOpenAgent?: (route: string) => void }) {
  // Merge live agent data with static dock entries
  const liveMap = new Map((agents || []).map((a: any) => [a.id?.toLowerCase(), a]));
  return (
    <div style={{ position: "absolute", bottom: 26, left: "50%", transform: "translateX(-50%)", zIndex: 30, display: "flex", gap: 8, padding: "8px 16px", background: "rgba(5,7,14,0.92)", backdropFilter: "blur(12px)", borderRadius: 18, border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 -4px 40px rgba(0,0,0,0.5)" }}>
      {DOCK_AGENTS.map(da => {
        const color = DOCK_AGENT_COLORS[da.id] || "#fff";
        const live = liveMap.get(da.id);
        const isOnline = live ? (live.status === "online" || live.status === "healthy") : da.id === "claude";
        return (
          <button
            key={da.id}
          ...
          >
            {/* Avatar circle */}
            <div style={{ width: 40, height: 40, borderRadius: "50%", background: `linear-gradient(145deg,${color}28,${color}12)`, border: `2px solid ${color}${isOnline ? "99" : "44"}`, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", boxShadow: isOnline ? `0 0 14px ${color}55` : "none" }}>
              <span style={{ color, fontSize: 13, fontWeight: 800, lineHeight: 1 }}>{da.name.slice(0,2).toUpperCase()}</span>
              {/* Online dot */}
              <div style={{ position: "absolute", bottom: 0, right: 0, width: 9, height: 9, borderRadius: "50%", background: isOnline ? "#22c55e" : "rgba(255,255,255,0.18)", border: "1.5px solid #030509", boxShadow: isOnline ? "0 0 6px #22c55e" : "none" }} />
            </div>
            <span style={{ fontSize: 7.5, fontWeight: 700, color: `${color}bb`, letterSpacing: ".08em", textTransform: "uppercase" }}>{da.name}</span>
          </button>
        );
      })}
    </div>
